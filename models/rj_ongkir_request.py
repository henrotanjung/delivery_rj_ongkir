...
class RjOngkirRequest():
    ...
    def set_package_detail_rj_ongkir(self, client, packages, packaging_type, namespace, ship_from, ship_to, cod_info):
        Packages = []
        for i, p in enumerate(packages):
            package = client.factory.create('{}:PackageType'.format(namespace))
            if hasattr(package, 'Packaging'):
    ...
    def get_shipping_price(self,total_weight, shipper, ship_from,
            ship_to, shipper_carrier, service_type, headers,conn):
        # user = self.env.user
        print (ship_from.city, 'shipppp from', shipper_carrier)
        company_address_id = ship_from.city #501 #user.company_id.city
        origin = company_address_id
        payload = f"origin={company_address_id}&destination={ship_to}&weight={total_weight}&courier={shipper_carrier}"

        print (type(payload), 'pay loadddddddddddddd')  # origin=114&destination=114&weight=0.0&courier=jne
    ...
    def get_shipping_price_old(self, shipment_info, packages, shipper, ship_from, ship_to, packaging_type, service_type, saturday_delivery, cod_info):
